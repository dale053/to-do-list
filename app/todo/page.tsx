      ...
      <div className="text-3xl">
        <section className="text-gray-600 body-font">
          <div className="container px-5 py-24 mx-auto flex flex-wrap items-start gap-10">
            {/* Add Todo Form */}
            <div className="rounded-lg p-8 flex flex-col md:ml-auto w-full md:w-5/12 mt-10 bg-slate-300">
            ...
            {/* Todo List Display */}
            <div className='p-8 flex flex-col w-full md:w-6/12 bg-green-100 rounded-lg mt-10'>
              {loading ? (
                <div className="text-center text-gray-700 text-lg">Loading...</div>
              ) : list.length === 0 ? (
                <div className="text-center text-gray-500 text-lg">No todos yet.</div>
              ) : (
                list.map((item, index) => (
                  <div className='w-full text-lg flex flex-row my-1 items-center' key={item._id}>
                    <div className="w-1/12">{index + 1}</div>
                    <div className="w-3/12">{item.title}</div>
                    <div className="w-6/12">{item.desc}</div>
                    <div className="w-2/12 flex flex-row justify-end">
                      <button
                        onClick={() => handleToggleTodo(item._id)}
                        className={`px-5 py-1 rounded text-white mr-2 ${
                          item.state
                      ...
                      <button
                        className="px-4 py-1 rounded text-white bg-red-400 hover:bg-red-500"
                        onClick={() => handleDeleteTodo(item._id)}
                      >
                        Delete
                      </button>
